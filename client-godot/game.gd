	...
	if GlobalNetwork.active:
		for id in GlobalNetwork.players:
			if player_ledger.has(id):
				...
				var player_scene = create_player(id, GlobalNetwork.players[id]['position'], GlobalNetwork.players[id]['rotation'], GlobalNetwork.players[id]['username'])
...
func create_player(id, position, rotation, player_name) -> Player:
	var new_player: Player = player_scene.instantiate()
	player_container.add_child(new_player)
	var p = new_player.spawn(int(id), Vector3(position.x, position.y, position.z), rotation, player_name)
	return p
